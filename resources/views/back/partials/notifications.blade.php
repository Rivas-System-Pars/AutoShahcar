                            ...
                            <a class="d-flex justify-content-between" href="{{ route('admin.notifications') }}">
                                <div class="media d-flex align-items-start">
                                    ...
                                    <div class="media-body">
                                        <h6 class="primary media-heading">سفارش جدید ثبت شد</h6>
                                        <small class="notification-text">{{ $notification->data['message'] }}</small>
                                    </div>
                                    <small><time class="media-meta">{{ jdate($notification->created_at)->ago() }}</time></small>
                                </div>
                            </a>
                        @elseif($notification->type == 'UserRegistered')
                            <a class="d-flex justify-content-between" href="{{ route('admin.notifications') }}">
                                <div class="media d-flex align-items-start">
                                    ...
                                    <div class="media-body">
                                        <h6 class="success media-heading">کاربر جدید ثبت نام کرد</h6>
                                        <small class="notification-text">{{ $notification->data['message'] }}</small>
                                    </div>
                                    <small><time class="media-meta">{{ jdate($notification->created_at)->ago() }}</time></small>
                                </div>
                        ...
                        @elseif($notification->type == 'ContactCreated')
                            <a class="d-flex justify-content-between" href="{{ route('admin.notifications') }}">
                                <div class="media d-flex align-items-start">
                                    ...
                                    <div class="media-body">
                                        <h6 class="info media-heading">پیام جدید دریافت شد</h6>
                                        <small class="notification-text">{{ $notification->data['message'] }}</small>
                                    </div>
                                    <small><time class="media-meta">{{ jdate($notification->created_at)->ago() }}</time></small>
                                </div>
