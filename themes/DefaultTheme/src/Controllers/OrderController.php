...
private function orderPaid(Order $order)
{
    ...
    if ($userPlan && $userPlan->end_date && Carbon::parse($userPlan->end_date)->gt($now)) {
        $end_date = Carbon::parse($userPlan->end_date)->addMonths($att);
    } else {
        $end_date = $att ? $now->copy()->addMonths($att) : null;
    }
