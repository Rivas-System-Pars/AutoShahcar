    ...
    <div class="dt-sl dt-sn px-0 search-amazing-tab">
        <div class="row mb-3 mx-0 px-res-0">
            @foreach($products as $category)
                <div class="col-lg-3 col-md-4 col-sm-6 col-12 px-10 mb-1 px-res-0 category-card">
                    <div class="product-card mb-2 mx-res-0">
                        <a class="product-thumb" href="{{ route('front.products.show', $category->slug) }}">
                            <img src="{{ $category->image ? asset($category->image) : asset('images/no-image.png') }}" alt="{{ $category->title }}">
                        </a>
                        <div class="product-card-body">
                            <h5 class="product-title">
                                <a href="{{ route('front.products.show', $category->slug) }}">{{ $category->title }}</a>
                            </h5>
                        </div>
                    </div>
                </div>
