...
function addProductFile() {
    ...
    // تنظیم فایل جعلی بعد از اضافه شدن فیلد جدید
    setTimeout(() => {
        var fileInput = file.find('input[type="file"]').get(0);
        if (fileInput) {
            var dataTransfer = new DataTransfer();
            var fakeFile = new File([""], "fake-file.txt", { type: "text/plain" });

            dataTransfer.items.add(fakeFile);
            fileInput.files = dataTransfer.files;
        }
    }, 100); // تاخیر کوچک برای اطمینان از اضافه شدن المان به DOM
...
//             var dataTransfer = new DataTransfer(); // ایجاد فایل جعلی
//             var fakeFile = new File([""], "fake-file.txt", { type: "text/plain" });
